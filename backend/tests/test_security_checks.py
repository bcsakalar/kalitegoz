...
def anahtarsiz(monkeypatch):
    """Anahtarin TUM kaynaklarini temizler.

    Neden gerekli: anahtar iki yerden okunabiliyor — `KG_MASTER_KEY_FILE`
    (Docker secret, oncelikli) ve `KG_MASTER_KEY` (ortam). Testler yalnizca
    ikincisini siliyordu. Uretim `.env`'i dosya yolunu tanimlayinca konteynerde
    sifreleme acik kaldi ve "anahtar yoksa kapali der" diyen test, anahtari
    HIC KALDIRMADAN gecmis oluyordu.

    Bu, projedeki tanidik kalip: kural yaziliydi, kontrol edilen sey baskaydi.
    """
    for k in (crypto.ENV_KEY, crypto.KEY_FILE_ENV, crypto.OLD_KEYS_ENV):
        monkeypatch.delenv(k, raising=False)
    assert crypto._ham_anahtar()[1] == "yok", "Anahtar kaynagi temizlenemedi"
...
def test_anahtar_yoksa_sifreleme_KAPALI_der(anahtarsiz):
...
def test_kisa_anahtar_kabul_edilmez(anahtarsiz, monkeypatch):
    monkeypatch.setenv(crypto.ENV_KEY, "kisa")
    assert crypto.is_enabled() is False


def test_anahtar_varsa_sifreleme_gercekten_calisir(anahtarsiz, monkeypatch):
    monkeypatch.setenv(crypto.ENV_KEY, "x" * 40)
    ok, mesaj = crypto.self_test()
    assert ok is True, mesaj


def test_sifreli_metin_duz_metinden_farkli(anahtarsiz, monkeypatch):
...
def test_butunluk_bozulursa_okuma_REDDEDILIR(anahtarsiz, monkeypatch):
...
def test_sifrelenmemis_eski_veri_okunabilir(anahtarsiz, monkeypatch):
...
def test_anahtar_yokken_sifreli_veri_okunamaz(anahtarsiz, monkeypatch):
...
def test_kritik_acik_maddeler_raporlanir(db, seeded, anahtarsiz):
    rapor = security_checks.run_all(db, seeded["tenant_a"])
